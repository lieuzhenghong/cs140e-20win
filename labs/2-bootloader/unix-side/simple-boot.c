...
void simple_boot(int fd, const uint8_t *buf, unsigned n) { 
    ...
    u32 code_crc = crc32(buf, n);

    // 1. reply to the GET_PROG_INFO
    put_uint32(fd, PUT_PROG_INFO);
    put_uint32(fd, ARMBASE);
    put_uint32(fd, n);
    put_uint32(fd, code_crc);

    // 2. drain any extra GET_PROG_INFOS
    unsigned buf_code;
    while ((buf_code = get_op(fd)) == GET_PROG_INFO);

    // 3. check that we received a GET_CODE 
    // also check that reply crc is the crc value sent
    demand(buf_code == GET_CODE, "Did not receive GET_CODE, %xd vs %xd", buf_code, GET_CODE);
    ck_eq32(fd, "Checksum does not match", code_crc, get_op(fd));

    // 4. handle it: send a PUT_CODE message.
